...
import './GuessWhoCurrentPlayerView.css';


export default function GuessWhoCurrentPlayerViewer ({selectedPlayer, submitPlayer, onclick, setPlayerMatches}) { 
    ...
    return (
        ...
        className={`current_row ${submitPlayer != undefined ? 'need_bg' : ""}`}> 
            <div className="currentPlayerView">
                {/* <span className="iteminrow"> {selectedPlayer['overall']} </span> */}
                <span className="iteminrow"> {selectedPlayer['position']} </span>
                <span className="iteminrow" > {selectedPlayer['short_name']} </span>
                <img className="guess_who_current_player_view_img" src={selectedPlayer['img_url']} />
